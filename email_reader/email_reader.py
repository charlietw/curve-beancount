...
class EmailReader:
    def __init__(self, scopes, address_to, token_dir, creds_dir):
        self.scopes = scopes
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                flow = InstalledAppFlow.from_client_secrets_file(
                    self.creds_dir, scopes=self.scopes
                )
    ...
    def get_emails_to(self, emails_to_retrieve, pattern=None):
        ...
        results = (
            ...
            .users()
            .messages()
            .list(userId="me", maxResults=emails_to_retrieve, q=query)
            .execute()
        )
    ...
    def get_label_id(self, label_str):
        label_id = (
            self.service().users().labels().get(userId="me", id=label_str).execute()
        )
        return label_id
    ...
    def move_email(self, message_id, label_from, label_to):
        ...
        """
        body = {
            "addLabelIds": [label_to],
            "removeLabelIds": [label_from],
        }
